...
import com.example.auth.application.JwtRepository
...
object SlickJwtRepository {
  def apply(secret: String, tokenExpiration: FiniteDuration = 24.hours): SlickJwtRepository =
    new SlickJwtRepository(secret, tokenExpiration)
}